...
- измена локалних конфигурација 
- играње игре
- провера интегритета преко DRM сервера
...
По својој природи, клијент је "глуп" - он нема сложену интерну логику, 
већ ће обраћа серверу (**Game Server**) на који је конектован. 
На серверу је покренута игра: ту се обрађује логика игре и симулација физике. 
При покретању, сервер учитава конфигурације из локалних датотека (**Server Config**). 
Сервер може по потреби да служи додатан садржај у виду специјалних асета које 
клијенти преузимају током конекције на сервер, као што је приказано на слици 3. Ти асети се чувају локално на серверу (**Server Data**), а при конекцији преузимају и код клијента.

![img](./custom-model.jpg)
_Слика 3. Counter-Strike 1.6 сервер који користи измењене моделе играча. Ове измене се примењују само на тај сервер. Злоупотребом ове функционалности може се спровести напад A1.2.2 који је опсисан у секцији 2._

Сервер за DRM (**DRM Server**) обезбеђује заштиту од пиратерије.
Реализован је као _always online DRM_ који захтева сталну конекцију са клијентима.
Клијенти периоднично шаљу потребне податке о свом идентитету које сервер за DRM валидира.

### 1.2. Ресурси и претње

На слици 4 приказан је дијаграм токова података, допуњен са одабраним ресурсима од значаја.

![img](./dfd_with_resources.drawio.png)

_Слика 4. Модификован дијаграм тока података игре, где су одређеним чворовима придружени значајни ресурси. Ресурси су репрезентовани идентификатором који одговара првој колони у табели 1._
...
У овом раду разматрана је претња T1 на ресурс R1. Разрађено стабло напада дато је на слици 5. 

![](./threat_decomposition.drawio.png)

_Слика 5. Стабло напада за претњу T1._

У наставку је дат кратак преглед свих претњи и напада са слике 5. Неки од напада су детаљније описани у секцији 3, а за остале је опис и графички пример дат испод.

Манипулација положајем играча подразумева неовлашћено читање и писање података о играчима у текућој игри. У играма FPS жанра, најпознатија злоупотреба ове претње је у виду ботова за нишан (_aimbot_) где се нишан играча аутоматски позиционира ка непријатељу на основу његове позиције. У овом раду _aimbot_ напади су груписани као напади на **_aimbot_ претњу - T1.1**. У табели 3 дат је преглед напада за ову претњу.
...
**ESP (_Extra Sensory Perception_) - T1.2** фамилија напада играчу приказује додатне информације о противницима - положај, енергија, муниција, ранг и сл. У случају T1 претње, фокус је само на положају играча. За разлику од _aimbot_ претње, ESP рањвост подразумева само неовлашћено читање података. Табела 4 описује издвојене нападе за ову претњу. Слика 6 деомнстрира напреднији ESP напад. 

![img](./esp_hack.png)
_Слика 6. ESP напад на игру Counter-Strike 2. Сваком играчу је додељен гранични оквир и информација о енергији, оружју и муницији._
...
| A1.3.2 | Нападач модификује брзину играча и тако брже стиже до неке локације. Пример напада дат је у слици 7. |

_Табела 5. Напади на претњу T1.3._

![img](./speedhack.gif)
_Слика 7. Speedhack напад на игру Counter-Strike 1.6._

Иако се у раду разматра нападач који је искључиво са клијентске стране,
напомињемо да нападач може бити и сервер. Напад на претњу Т7 је делотворнији ако
се спроведе са серверске стране. Исто тако, сервирање асета као што је описано у
секцији 1 и приказано на слици 3 би се могло злоупотребити у случају да не
постоји валидација садржаја са клијентске стране. На пример, уместо да сервер
пошаље модификоан модел играча, он шаље скрипту која отвара reverse shell.
Међутим, у пракси се показало да су овакви напади ретки јер погони за игре, као
много сложенији софтвер од апликативних сервера, подржавају рад са фајловима
само одређеног типа и интерне структуре.
...
Неке игре аутоматски померају играчев нишан ка непријатељу ако играч посматра
непријатеља унутар неког прага. Ова функционалност се зове _aim assist_ и
саставни је део многих конзолних игара.

Праг за _aim assist_ је конфигурабилна вредност: играчи је постављају на
вредност која њима оговара. У зависности од игре, могуће је и да сервер
контролише ову вредност за све клијенте. Играч у конфигурацијама игре
може да промени ову вредност прага, у виду слајдера.

Слајдер, као елемент графичког корисничког интерфејса, сам по себи ограничава
вредност прага на неки минимум и максимум. Рањивост потиче из чињенице да се
вредност прага контролише искључиво кроз GUI елемент.

Овај напад подразумева измену прага тако да се _aim assist_ активира чак и ако
играч не гледа ни приближно у правцу непријатеља. Напад би се спровео директном
променом вредности прага у радној меморији, користећи алате попут _CheatEngine_
или _CodeBreaker_. Алтернативни напад би подразумевао да се у трајном складишту
модификује вредност прага који се учитава при покретању клијента.
...
добаве податке о играчима са сервера. На слици 8 приказан је пример овог напада.

![](./hook-aimbot.gif)

_Слика 8. Hook aimbot за игру Counter-Strike 1.6._
...
(који немају везе са системским позивима у оперативним системима).
...
Пресретањем `syscall` функције, било би могуће добавити положај ентитета
и тако имплементирати _aimbot_. Исечак кода испод приказује поједностављен
_aimbot_ који ради по овом принципу. На слици 9 илустрован је ток напада.
...
_Слика 9. Дијаграм секвенце aimbot напада преко DLL инјекције._
...
Једини начин да се митигује било који _aimbot_ напад је **мониторинг понашања играча**.
Ово се може спровести аутоматски (провера да ли се играч окреће абнормално брзо) или 
мануелно (посматрањем игре у току или снимљеног _replay_-а).
Због особине универзалности, овај вид митигације покрива нападе у ширину, али не и у
дубину - развојем ботова постаје све теже препознати лажног од стварног играча, а технике
попут свичева за лаг (_lagswitch_) се могу користити као изговор у случају оптужбе за варање.

### A1.1.2 Pixelbot

_Pixelbot_ је софистицирана варијанта _aimbot_ напада која искључиво користи приказ клијентске апликације.
На слици 10 приказан је _pixelbot_ напад.

![](./pixelbot.gif)

_Слика 10. Pixelbot напад на игру Counter-Strike: Global Offensive. Сваком играчу је додељен гранични оквир и тим којем играч припада (на пример, `t` је ознака за тим терориста)._

#### Анализа напада

У овом нападу се користи екстерни програм који чита приказ са клијентске апликације у виду матрице пиксела.
Техникама рачунарске визије се из тих пиксела издвајају потенцијални играчи.
На основу предикција модела се доноси одлука о томе да ли клијент треба да преусмери нишан ка некој од мета.

Следећи исечак представља псеудокод _pixelbot_ напада по угледу на решење дато у [[10]](https://github.com/methamphetaminelab/AI_AIMBOT_CS1.6).
...
def main():
    ...
    cs_window = get_window_with_title("Counter-Strike")
    while True:
        ss = screenshot()
        ss = ss.crop(cs_window.get_rect())

        results = model(ss)

        for res in results:
            if res["label"] == enemy_team:
                enemy_center = res["center"]
                mouse_dx = ...
                mouse_dy = ...

                win32.send_event(cs_window, MOUSE_MOVE, (mouse_dx, mouse_dy))
```

#### Безбедносне контроле

_Pixelbot_ напад не захтева инјекцију кода у клијентској апликацији чиме се заобилазе
прве две технике митигације из претходног напада (провера интегритета и кернел драјвери). 

Процедуре кроз које се спроводи овај напад су добављање пиксела из прозора
клијента и слање покрета миша ка клијенту. Добављање пиксела би се могло
детектовати пресретањем графичких драјвера. Слање покрета миша другом процесу би
се потенцијално могло детектовати на нивоу кернела. Међутим, ниједан од ова
приступа није практичан: оба се своде на комуникацију са кернелом оперативног
система. Стога, употребом таквог софтвера за детекцију би се проширио вектор
напада. Чак и када би ове митигације постојале и биле у употреби, напад A1.1.3
из обе заобилази.
...
[10] https://github.com/methamphetaminelab/AI_AIMBOT_CS1.6 Датум приступа 14.12.2024.